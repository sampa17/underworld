...
package net.mcreator.project.world.biome;
...
import net.mcreator.project.entity.PrimitifEntity;
...
public class BedrockbiomeBiome extends ProjectModElements.ModElement {
	@ObjectHolder("project:bedrockbiome")
	public static final CustomBiome biome = null;
	public BedrockbiomeBiome(ProjectModElements instance) {
		super(instance, 3);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, new CustomTreeFeature()
					.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(UnderstoneBlock.block.getDefaultState()),
			...
			this.addSpawn(EntityClassification.CREATURE, new Biome.SpawnListEntry(PrimitifEntity.entity, 15, 1, 5));
		}
	}
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == Blocks.BEDROCK.getDefaultState().getBlock() || ground == UnderstoneBlock.block.getDefaultState().getBlock())
							&& (ground2 == Blocks.BEDROCK.getDefaultState().getBlock()
									|| ground2 == UnderstoneBlock.block.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), UnderstoneBlock.block.getDefaultState(), bbox);
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, UnderstoneBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == UnderstoneBlock.block.getDefaultState().getBlock()
					|| blockType == Blocks.AIR.getDefaultState().getBlock() || blockType == Blocks.BEDROCK.getDefaultState().getBlock()
					|| blockType == UnderstoneBlock.block.getDefaultState().getBlock();
		}
